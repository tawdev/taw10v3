...
interface LanguageContextType {
  ...
  setLanguageOnly: (lang: Language) => void;
  t: (key: string) => string;
}
...
export const LanguageProvider: React.FC<{ children: React.ReactNode, initialLanguage?: string }> = ({ children, initialLanguage = "FR" }) => {
  ...
  // Persist language without triggering a URL redirect — safe to use inside the dashboard
  const setLanguageOnly = (lang: Language) => {
    setLanguageState(lang);
    localStorage.setItem("language", lang);
  ...
  return (
    <LanguageContext.Provider value={{ language, setLanguage, setLanguageOnly, t }}>
      {children}
