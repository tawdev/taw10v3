...
export default function Team() {
  ...
  useEffect(() => {
    ...
    teamService
      .publicList()
      .then((members) => {
        if (mounted) setDynamicMembers(members);
      })
      .catch(() => {
        if (mounted) setDynamicMembers([]);
      });
  ...
  const languageKey = language.toLowerCase() as 'fr' | 'en' | 'ar';
  const fallbackMembers = useMemo(
    () => [
      { name: t("team.ceo_name"), role: t("team.ceo"), desc: t("team.ceo_desc"), img: "/hicham.jpeg" },
      { name: t("team.operational_name"), role: t("team.operational"), desc: t("team.operational_desc"), img: "/afafe-khlifal.png" },
      { name: t("team.commercial_name"), role: t("team.commercial"), desc: t("team.commercial_desc"), img: "/salma-aaouad.png" },
    ],
    [t],
  );

  const teamMembers = dynamicMembers.length
    ? dynamicMembers.map((member) => ({
        name: member[`name_${languageKey}`] || member.name_fr,
        role: member[`role_${languageKey}`] || member.role_fr,
        desc: member[`description_${languageKey}`] || member.description_fr,
        img: member.imageUrl,
      }))
    : fallbackMembers;
