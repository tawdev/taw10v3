...
export default function Expertise() {
  ...
  const [dynamicServices, setDynamicServices] = useState<ServiceOffering[]>([]);
  ...
  useEffect(() => {
    ...
    servicesService
      .publicList()
      .then((items) => {
        if (mounted) setDynamicServices(items);
      })
      .catch(() => {
        if (mounted) setDynamicServices([]);
      });
  ...
  const fallbackServices = useMemo(
    () => [
      { title: t("service_card.premium_title"), icon: "location_on", slug: "domiciliation", desc: t("service_card.premium_desc"), imageUrl: "/luxury_marrakech_office_hero_1775496536100.png" },
      { title: t("service_card.creation_title"), icon: "rocket_launch", slug: "creation-entreprise", desc: t("service_card.creation_desc"), imageUrl: "/leadership-photo.jpeg" },
      { title: t("service_card.secretary_title"), icon: "forward_to_inbox", slug: "secretariat", desc: t("service_card.secretary_desc"), imageUrl: "/team-member-1.jpeg" },
      { title: t("service_card.legal_title"), icon: "gavel", slug: "accompagnement-juridique", desc: t("service_card.legal_desc"), imageUrl: "/team-member-2.jpeg" },
      { title: t("service_card.support_title"), icon: "support_agent", slug: "support-administratif", desc: t("service_card.support_desc"), imageUrl: "/blog-avantage.jpg" },
      { title: t("service_card.strategic_title"), icon: "insights", slug: "conseil-strategique", desc: t("service_card.strategic_desc"), imageUrl: "/hicham.jpeg" },
    ],
    [t],
  );
  ...
  const services = dynamicServices.length
    ? dynamicServices.map((item) => ({
        ...
        icon: item.icon,
        slug: item.slug,
        desc: item[`description_${lang}`] || item.description_fr,
        ...
        <div className="max-w-7xl mx-auto px-8 relative z-10">
          ...
          >
            {services.map((item, idx) => (
              <motion.div 
                key={idx}
                className="bg-[#fcf9f6] rounded-3xl border border-[#dab055]/10 group relative overflow-hidden flex flex-col h-full shadow-sm"
                variants={fadeInUp}
                whileHover={{ 
                  y: -8, 
