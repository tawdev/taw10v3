...
const Toggle: React.FC<ToggleProps> = ({ id, checked, disabled = false, onChange, label }) => (
  <button
    ...
    className={`relative inline-flex h-5 w-10 shrink-0 items-center rounded-full border transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#dab055] focus-visible:ring-offset-2 focus-visible:ring-offset-[#1c1c1b]
      ${disabled
        ? "cursor-not-allowed border-[#dab055]/60 bg-[#dab055]/30"
        : checked
          ? "cursor-pointer border-[#dab055] bg-[#dab055]/20"
  ...
  >
    <span
      className={`inline-block h-3.5 w-3.5 rounded-full transition-all duration-300 shadow-md
        ${checked
          ? "translate-x-5 bg-[#dab055] shadow-[0_0_8px_rgba(218,176,85,0.6)]"
...
const CookieConsent: React.FC = () => {
  ...
  return (
    <>
      ...
      >
        <div className="relative w-full max-w-xl overflow-hidden rounded-2xl border border-[#dab055]/20 bg-[#1c1c1b] shadow-[0_32px_80px_rgba(0,0,0,0.7),0_0_0_1px_rgba(218,176,85,0.08)] text-white">

          {/* ── Top accent line */}
          <div className="h-[2px] w-full bg-gradient-to-r from-transparent via-[#dab055] to-transparent opacity-60" />
          ...
          <div className="flex border-b border-white/10 px-6">
            {tabs.map((tab) => (
              ...
              >
                {tab.label}
                {activeTab === tab.id && (
                  <span className="absolute bottom-0 left-0 right-0 h-[2px] rounded-t-full bg-[#dab055] shadow-[0_0_8px_rgba(218,176,85,0.7)]" />
                )}
              </button>
            ))}
          </div>
          ...
          <div className="overflow-y-auto max-h-[60vh] px-6 py-5 space-y-5 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                    ...
                    >
                      <summary className="flex cursor-pointer items-center justify-between px-4 py-3 list-none select-none">
                        <div className="flex items-center gap-3">
                          <span
                            className={`h-2 w-2 rounded-full shrink-0 ${prefs[key] ? "bg-[#dab055] shadow-[0_0_6px_rgba(218,176,85,0.7)]" : "bg-white/20"}`}
                          />
                          <span className="text-[13px] font-semibold text-white/80">{label}</span>
                          {disabled && (
                            <span className="rounded-full bg-[#dab055]/15 px-2 py-0.5 text-[10px] font-bold text-[#dab055] uppercase tracking-wide">
                              Always on
                            </span>
                          )}
                        </div>
            ...
            {activeTab === "about" && (
              <div className="space-y-4 animate-[fadeIn_0.25s_ease]">
                ...
                <div className="rounded-xl border border-[#dab055]/20 bg-[#dab055]/5 p-4">
                  <p className="text-[12px] text-[#dab055]/80 leading-relaxed">
                    Your consent applies to the following domains:{" "}
                    <span className="font-semibold text-[#dab055]">taw10.ma</span>
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="flex flex-col gap-2 border-t border-white/[0.07] bg-[#06101d] px-6 py-4 sm:flex-row sm:items-center sm:justify-end">
            ...
            <button
              ...
              className="rounded-lg border border-[#dab055]/30 bg-[#dab055]/10 px-4 py-2.5 text-[12px] font-semibold text-[#dab055] transition-all duration-200 hover:bg-[#dab055]/20 hover:border-[#dab055]/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#dab055]/50"
            >
              Allow selection
            </button>
            <button
              onClick={handleAllowAll}
              className="rounded-lg bg-[#dab055] px-4 py-2.5 text-[12px] font-bold text-[#1c1c1b] transition-all duration-200 hover:bg-[#ebd380] hover:shadow-[0_0_20px_rgba(218,176,85,0.35)] active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#dab055]"
            >
              Allow all cookies
            </button>
          </div>
